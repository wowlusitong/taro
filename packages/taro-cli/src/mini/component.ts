...
export function isFileToBeTaroComponent (code: string, sourcePath: string, outputPath: string) {
  const { buildAdapter, sourceDir, constantsReplaceList, jsxAttributeNameReplace, alias } = getBuildData()
...
export async function buildSingleComponent (
  ...
  if (!component) {
    printLog(
      ...
      '组件错误',
      `组件${_.upperFirst(_.camelCase(componentObj.name))}路径错误，请检查！（可能原因是导出的组件名不正确）`
    )
    return {
      js: '',
      wxss: '',
  ...
  let outputComponentShowPath = componentShowPath.replace(
    isComponentFromNodeModules ? NODE_MODULES : sourceDirName,
    buildConfig.outputDirName || outputDirName
  )
  ...
  const outputComponentJSPath = component
    .replace(sourceDirPath, buildConfig.outputDir || buildOutputDir)
    .replace(extnameExpRegOf(component), outputFilesTypes.SCRIPT)
  const outputComponentWXMLPath = outputComponentJSPath.replace(
    extnameExpRegOf(outputComponentJSPath),
    outputFilesTypes.TEMPL
  )
  const outputComponentWXSSPath = outputComponentJSPath.replace(
    extnameExpRegOf(outputComponentJSPath),
    outputFilesTypes.STYLE
  )
  const outputComponentJSONPath = outputComponentJSPath.replace(
    extnameExpRegOf(outputComponentJSPath),
    outputFilesTypes.CONFIG
  )
  ...
  try {
    ...
    if (!isTaroComponentRes.isTaroComponent) {
      ...
      const componentRealPath = parseComponentExportAst(
        transformResult.ast,
        componentObj.name as string,
        component,
        componentObj.type as string
      )
    ...
    if (
      isComponentHasBeenBuilt(componentObj.path as string) &&
      componentsBuildResult.get(componentObj.path as string)
    ) {
    ...
    const res = parseAst(
      PARSE_AST_TYPE.COMPONENT,
      transformResult.ast,
      componentDepComponents,
      component,
      outputComponentJSPath,
      buildConfig.npmSkip
    )
    ...
    if (usingComponents && !isEmptyObject(usingComponents)) {
      ...
      transfromNativeComponents(
        outputComponentJSONPath.replace(buildConfig.outputDir || buildOutputDir, sourceDirPath),
        res.configObj
      )
    }
    ...
    if (!isQuickApp) {
      ...
      const importCustomComponents = new Set(
        realComponentsPathList.map(item => {
          return {
            path: promoteRelativePath(path.relative(component, item.path as string)).replace(
    ...
    if (realComponentsPathList.length) {
      res.scriptFiles = res.scriptFiles
        ...
        .filter(item => item)
      realComponentsPathList = realComponentsPathList.filter(
        item => !isComponentHasBeenBuilt(item.path as string) || notTaroComponents.has(item.path as string)
      )
    ...
    if (!isQuickApp) {
      fs.writeFileSync(
        outputComponentJSONPath,
        JSON.stringify(
          _.merge({}, buildUsingComponents(component, componentDepComponents, true), res.configObj),
          null,
          2
        )
      )
      printLog(
        ...
        '组件配置',
        `${outputDirName}/${outputComponentShowPath}${outputFilesTypes.CONFIG}`
      )
      fs.writeFileSync(outputComponentJSPath, resCode)
      printLog(
        processTypeEnum.GENERATE,
        '组件逻辑',
        `${outputDirName}/${outputComponentShowPath}${outputFilesTypes.SCRIPT}`
      )
      fs.writeFileSync(outputComponentWXMLPath, componentWXMLContent)
      processNativeWxml(
        outputComponentWXMLPath.replace(outputDir, sourceDir),
        componentWXMLContent,
        outputComponentWXMLPath
      )
      printLog(
        processTypeEnum.GENERATE,
        '组件模板',
        `${outputDirName}/${outputComponentShowPath}${outputFilesTypes.TEMPL}`
      )
    }
    // 编译依赖的脚本文件
    ...
    if (
      ...
      isDifferentArray(depComponents.get(component) || [], componentDepComponents)
    ) {
      printLog(
        processTypeEnum.GENERATE,
        '组件样式',
        `${outputDirName}/${outputComponentShowPath}${outputFilesTypes.STYLE}`
      )
